---
local: summary-of-the-models
sections:
- local: decoders-or-autoregressive-models
  sections:
  - local: original-gpt
    title: Original GPT
  - local: gpt2
  ...
  - local: xlnet
    title: XLNet
  title: Decoders or autoregressive models
- local: encoders-or-autoencoding-models
  sections:
  - local: bert
    title: BERT
  - local: albert
  ...
  - local: longformer
    title: Longformer
  title: Encoders or autoencoding models
- local: sequencetosequence-models
  sections:
  - local: bart
    title: BART
  - local: pegasus
...
- local: retrievalbased-models
  sections:
  - local: dpr
    title: DPR
  - local: rag
    title: RAG
  title: Retrieval-based models
- local: more-technical-aspects
  sections:
  - local: full-vs-sparse-attention
    title: Full vs sparse attention
  - local: other-tricks
    title: Other tricks
  title: More technical aspects
...
<h1 id="summary-of-the-models">Summary of the models</h1>
...
models. You can check them more in detail in their respective documentation. Also check out the [pretrained model
page](/docs/transformers/master/en/pretrained_models) to see the checkpoints available for each type of model and all [the community models](https://huggingface.co/models).

Each one of the models in the library falls into one of the following categories:

- [autoregressive-models](/docs/transformers/master/en/model_summary#autoregressive-models)
- [autoencoding-models](/docs/transformers/master/en/model_summary#autoencoding-models)
- [seq-to-seq-models](/docs/transformers/master/en/model_summary#seq-to-seq-models)
- [multimodal-models](/docs/transformers/master/en/model_summary#multimodal-models)
- [retrieval-based-models](/docs/transformers/master/en/model_summary#retrieval-based-models)
...
<h2 id="decoders-or-autoregressive-models">Decoders or autoregressive models</h2>
...
<h3 id="original-gpt">Original GPT</h3>
...
<h3 id="transformerxl">Transformer-XL</h3>
...
- Use [Axial position encoding](/docs/transformers/master/en/model_summary#axial-pos-encoding) (see below for more details). It’s a mechanism to avoid
  having a huge positional encoding matrix (when the sequence length is very big) by factorizing it into smaller
  matrices.
- Replace traditional attention by [LSH (local-sensitive hashing) attention](/docs/transformers/master/en/model_summary#lsh-attention) (see below for more
...
<h2 id="encoders-or-autoencoding-models">Encoders or autoencoding models</h2>
...
[ConvBERT: Improving BERT with Span-based Dynamic Convolution](https://arxiv.org/abs/1910.01108), Zihang Jiang,
...
Checkpoints refer to which method was used for pretraining by having _clm_, _mlm_ or _mlm-tlm_ in their names. On top
...
<h3 id="xlmroberta">XLM-RoBERTa</h3>
...
<h3 id="funnel-transformer">Funnel Transformer</h3>
...
[local attention section](/docs/transformers/master/en/model_summary#local-attention) for more information.
...
<h2 id="sequencetosequence-models">Sequence-to-sequence models</h2>
...
<h3 id="prophetnet">ProphetNet</h3>
...
<h3 id="xlmprophetnet">XLM-ProphetNet</h3>
...
<!--More information in this [model documentation](/docs/transformers/master/en/model_doc/mmbt). TODO: write this page
-->

<a id='retrieval-based-models'></a>

<h2 id="retrievalbased-models">Retrieval-based models</h2>
...
<h2 id="more-technical-aspects">More technical aspects</h2>

<h3 id="full-vs-sparse-attention">Full vs sparse attention</h3>
...
[Reformer](/docs/transformers/master/en/model_summary#reformer) uses LSH attention. In the softmax(QK^t), only the biggest elements (in the softmax
...
[Longformer](/docs/transformers/master/en/model_summary#longformer) uses local attention: often, the local context (e.g., what are the two tokens to the
...
<img scale="50 %" align="center" src="/docs/transformers/master/en/imgs/local_attention_mask.png"/>
...
[Reformer](/docs/transformers/master/en/model_summary#reformer) uses axial positional encodings: in traditional transformer models, the positional encoding
