...
We have seen in the [training tutorial](/docs/transformers/v4.14.1/en/training): how to fine-tune a model on a given task. You have probably
done something similar on your task, either using the model directly in your own training loop or using the
`Trainer`/`TFTrainer` class. Let's see how you can share the result on the
...
account by going on _huggingface.co &amp;lt;https://huggingface.co/>_, click on your avatar on the top left corner, then on
_Edit profile_ on the left, just beneath your profile picture. In the submenu _API Tokens_, you will find your API
...
If you want to create a repo under a specific organization, you should add a _--organization_ flag:
...
PyTorch _and_ TensorFlow checkpoints to make it easier to use (if you skip this step, users will still be able to load
...
or removing TF. For instance, if you trained a [DistilBertForSequenceClassification](/docs/transformers/v4.14.1/en/model_doc/distilbert#transformers.DistilBertForSequenceClassification), try to type

```
>>> from transformers import TFDistilBertForSequenceClassification
```

and if you trained a [TFDistilBertForSequenceClassification](/docs/transformers/v4.14.1/en/model_doc/distilbert#transformers.TFDistilBertForSequenceClassification), try to type
...
- a _config.json_ file, which saves the [configuration](/docs/transformers/v4.14.1/en/main_classes/configuration) of your model ;
- a _pytorch_model.bin_ file, which is the PyTorch checkpoint (unless you can't have it for some reason) ;
- a _tf_model.h5_ file, which is the TensorFlow checkpoint (unless you can't have it for some reason) ;
- a _special_tokens_map.json_, which is part of your [tokenizer](/docs/transformers/v4.14.1/en/main_classes/tokenizer) save;
- a _tokenizer_config.json_, which is part of your [tokenizer](/docs/transformers/v4.14.1/en/main_classes/tokenizer) save;
- files named _vocab.json_, _vocab.txt_, _merges.txt_, or similar, which contain the vocabulary of your tokenizer, part
  of your [tokenizer](/docs/transformers/v4.14.1/en/main_classes/tokenizer) save;
- maybe a _added_tokens.json_, which is part of your [tokenizer](/docs/transformers/v4.14.1/en/main_classes/tokenizer) save.
...
Model cards used to live in the 🤗 Transformers repo under _model_cards/_, but for consistency and scalability we
...
First you need to install _git-lfs_ in the environment used by the notebook:
