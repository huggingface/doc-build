...
`AwesomeModel` should subclass [PreTrainedModel](/docs/transformers/master/en/main_classes/model#transformers.PreTrainedModel) and `AwesomeConfig` should subclass [PretrainedConfig](/docs/transformers/master/en/main_classes/configuration#transformers.PretrainedConfig). The
...
[AutoConfig](/docs/transformers/master/en/model_doc/auto#transformers.AutoConfig)) but it's different for models. Your custom model could be suitable for sequence classification (in
