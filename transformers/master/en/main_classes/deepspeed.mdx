...
1. Integration of the core DeepSpeed features via [Trainer](/docs/transformers/master/en/main_classes/trainer#transformers.Trainer). This is an everything-done-for-you type
...
To deploy DeepSpeed with one GPU adjust the [Trainer](/docs/transformers/master/en/main_classes/trainer#transformers.Trainer) command line arguments as follows:
...
Since *"auto"* is used the [Trainer](/docs/transformers/master/en/main_classes/trainer#transformers.Trainer) arguments will set the correct values in the configuration
...
[TrainingArguments](/docs/transformers/master/en/main_classes/trainer#transformers.TrainingArguments) object if the passed DeepSpeed configuration file contains ZeRO-3 config
...
- DeepSpeed works with the PyTorch [Trainer](/docs/transformers/master/en/main_classes/trainer#transformers.Trainer) but not TF `TFTrainer`
...
The [HfDeepSpeedConfig](/docs/transformers/master/en/main_classes/deepspeed#transformers.deepspeed.HfDeepSpeedConfig) is used to integrate Deepspeed into the 🤗 Transformers core
functionality, when [Trainer](/docs/transformers/master/en/main_classes/trainer#transformers.Trainer) is not used. The only thing that it does is handling Deepspeed ZeRO 3 param gathering and automatically splitting the model onto multiple gpus during `from_pretrained` call. Everything else you have to do by yourself.
...
[HfDeepSpeedConfig](/docs/transformers/master/en/main_classes/deepspeed#transformers.deepspeed.HfDeepSpeedConfig) object before instantiating the model.
...
[Trainer](/docs/transformers/master/en/main_classes/trainer#transformers.Trainer) uses the `HfTrainerDeepSpeedConfig` subclass instead. That subclass has logic to sync the configuration
with values of [TrainingArguments](/docs/transformers/master/en/main_classes/trainer#transformers.TrainingArguments) by replacing special placeholder values: `"auto"`. Without this special logic
...
<div class="docstring">
<docstring><name>is\_false</name><anchor>transformers.deepspeed.HfDeepSpeedConfig.is_false</anchor><source>https://github.com/huggingface/transformers/blob/master/src/transformers/deepspeed.py#L146</source><parameters>[{"name": "ds_key_long", "val": ""}]</parameters></docstring>
...
Here is an example of how one could do DeepSpeed ZeRO Inference without using [Trainer](/docs/transformers/master/en/main_classes/trainer#transformers.Trainer) when one can't fit a model onto a single GPU. The solution includes using additional GPUs or/and offloading GPU memory to CPU memory.
