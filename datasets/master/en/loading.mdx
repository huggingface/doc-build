...
- local: slice-splits
  sections:
  - local: percent-slicing-and-rounding
    title: Percent slicing and rounding
  title: Slice splits
...
- local: metrics
  sections:
  - local: load-configurations
    title: Load configurations
  - local: distributed-setup
...
<h2 id="hugging-face-hub">Hugging Face Hub</h2>
...
First, create a dataset repository and upload your data files. Then you can use [datasets.load_dataset()](/docs/datasets/master/en/package_reference/loading_methods#datasets.load_dataset) like you learned in the tutorial. For example, load the files from this [demo repository](https://huggingface.co/datasets/lhoestq/demo1) by providing the repository namespace and dataset name:
...
```py
>>> data_files = &amp;lcub;"train": "train.csv", "test": "test.csv"}
...
```py
>>> data_files = &amp;lcub;"validation": "en/c4-validation.*.json.gz"}
>>> c4_validation = load_dataset("allenai/c4", data_files=data_files, split="validation")
```

<h2 id="local-and-remote-files">Local and remote files</h2>

🤗 Datasets can be loaded from local files stored on your computer, and also from remote files. The datasets are most likely stored as a `csv`, `json`, `txt` or `parquet` file. The [datasets.load_dataset()](/docs/datasets/master/en/package_reference/loading_methods#datasets.load_dataset) method is able to load each of these file types.
...
```py
>>> dataset = load_dataset('csv', data_files=&amp;lcub;'train': ['my_train_file_1.csv', 'my_train_file_2.csv'] 'test': 'my_test_file.csv'})
```
...
>>> dataset = load_dataset('csv', data_files=&amp;lcub;'train': base_url + 'train.csv', 'test': base_url + 'test.csv'})
```
...
>>> data_files = &amp;lcub;"train": url}
>>> dataset = load_dataset("csv", data_files=data_files)
```

<h3 id="json">JSON</h3>

JSON files are loaded directly with [datasets.load_dataset()](/docs/datasets/master/en/package_reference/loading_methods#datasets.load_dataset) as shown below:
...
```py
&amp;lcub;"a": 1, "b": 2.0, "c": "foo", "d": false}
&amp;lcub;"a": 4, "b": -5.5, "c": null, "d": true}
```
...
```py
&amp;lcub;"version": "0.1.0",
    "data": [&amp;lcub;"a": 1, "b": 2.0, "c": "foo", "d": false},
            &amp;lcub;"a": 4, "b": -5.5, "c": null, "d": true}]
}
...
>>> dataset = load_dataset('json', data_files=&amp;lcub;'train': base_url + 'train-v1.1.json', 'validation': base_url + 'dev-v1.1.json'}, field="data")
```
...
```py
>>> from datasets import load_dataset
>>> dataset = load_dataset('text', data_files=&amp;lcub;'train': ['my_text_1.txt', 'my_text_2.txt'], 'test': 'my_test_file.txt'})
```
...
```py
>>> from datasets import load_dataset
>>> dataset = load_dataset("parquet", data_files=&amp;lcub;'train': 'train.parquet', 'test': 'test.parquet'})
```
...
>>> data_files = &amp;lcub;"train": base_url + "wikipedia-train.parquet"}
>>> wiki = load_dataset("parquet", data_files=data_files, split="train")
```

<h2 id="inmemory-data">In-memory data</h2>

🤗 Datasets will also allow you to create a [datasets.Dataset](/docs/datasets/master/en/package_reference/main_classes#datasets.Dataset) directly from in-memory data structures like Python dictionaries and Pandas DataFrames.

<h3 id="python-dictionary">Python dictionary</h3>

Load Python dictionaries with [datasets.Dataset.from_dict()](/docs/datasets/master/en/package_reference/main_classes#datasets.Dataset.from_dict):

```py
>>> from datasets import Dataset
>>> my_dict = &amp;lcub;"a": [1, 2, 3]}
>>> dataset = Dataset.from_dict(my_dict)
```

<h3 id="pandas-dataframe">Pandas DataFrame</h3>

Load Pandas DataFrames with [datasets.Dataset.from_pandas()](/docs/datasets/master/en/package_reference/main_classes#datasets.Dataset.from_pandas):

```py
>>> from datasets import Dataset
>>> import pandas as pd
>>> df = pd.DataFrame(&amp;lcub;"a": [1, 2, 3]})
>>> dataset = Dataset.from_pandas(df)
```

<Tip warning=&amp;lcub;true}>

An object data type in [pandas.Series](https://pandas.pydata.org/docs/reference/api/pandas.Series.html) doesn't always carry enough information for Arrow to automatically infer a data type. For example, if a DataFrame is of length 0 or the Series only contains None/nan objects, the type is set to null. Avoid potential errors by constructing an explicit schema with [datasets.Features](/docs/datasets/master/en/package_reference/main_classes#datasets.Features) using the `from_dict` or `from_pandas` methods. See the [troubleshoot](#troubleshoot) for more details on how to explicitly specify your own features.
...
<h2 id="slice-splits">Slice splits</h2>

For even greater control over how to load a split, you can choose to only load specific slices of a split. There are two options for slicing a split: using strings or [datasets.ReadInstruction](/docs/datasets/master/en/package_reference/builder_classes#datasets.ReadInstruction). Strings are more compact and readable for simple cases, while [datasets.ReadInstruction](/docs/datasets/master/en/package_reference/builder_classes#datasets.ReadInstruction) is easier to use with variable slicing parameters.
...
>>> vals_ds = datasets.load_dataset('bookcorpus', split=[f'train[&amp;lcub;k}%:&amp;lcub;k+10}%]' for k in range(0, 100, 10)])
>>> trains_ds = datasets.load_dataset('bookcorpus', split=[f'train[:&amp;lcub;k}%]+train[&amp;lcub;k+10}%:]' for k in range(0, 100, 10)])
===STRINGAPI-READINSTRUCTION-SPLIT===
>>> # 10-fold cross-validation (see also next section on rounding behavior):
>>> # The validation datasets are each going to be 10%:
>>> # [0%:10%], [10%:20%], ..., [90%:100%].
...
<h3 id="percent-slicing-and-rounding">Percent slicing and rounding</h3>
...
<h3 id="specify-features">Specify features</h3>

When you create a dataset from local files, the [datasets.Features](/docs/datasets/master/en/package_reference/main_classes#datasets.Features) are automatically inferred by [Apache Arrow](https://arrow.apache.org/docs/). However, the features of the dataset may not always align with your expectations or you may want to define the features yourself. 

The following example shows how you can add custom labels with [datasets.ClassLabel](/docs/datasets/master/en/package_reference/main_classes#datasets.ClassLabel). First, define your own labels using the [datasets.Features](/docs/datasets/master/en/package_reference/main_classes#datasets.Features) class:

```py
>>> class_names = ["sadness", "joy", "love", "anger", "fear", "surprise"]
>>> emotion_features = Features(&amp;lcub;'text': Value('string'), 'label': ClassLabel(names=class_names)})
```

Next, specify the `features` argument in [datasets.load_dataset()](/docs/datasets/master/en/package_reference/loading_methods#datasets.load_dataset) with the features you just created:
...
&amp;lcub;'text': Value(dtype='string', id=None),
...
<h3 id="load-configurations">Load configurations</h3>
...
<h3 id="distributed-setup">Distributed setup</h3>
...
3. Load your metric with [datasets.load_metric()](/docs/datasets/master/en/package_reference/loading_methods#datasets.load_metric) with these arguments:
...
Once you've loaded a metric for distributed usage, you can compute the metric as usual. Behind the scenes, [datasets.Metric.compute()](/docs/datasets/master/en/package_reference/main_classes#datasets.Metric.compute) gathers all the predictions and references from the nodes, and computes the final metric.
