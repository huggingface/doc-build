...
- **axis** (`&amp;lcub;0, 1}`, default `0`, meaning over rows) --
  ...
  *New in version 1.6.0*</paramsdesc><paramgroups>0</paramgroups><rettype>`datasets.table.Table` that is the concatenated table</rettype><retdesc>If the number of input tables is > 1, then the returned table is a `datasets.table.ConcatenationTable`.
