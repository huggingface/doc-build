...
- local: select-a-split
  title: Select a split
...
<h1 id="hugging-face-hub">Hugging Face Hub</h1>
...
Before you take the time to download a dataset, it is often helpful to quickly get all the relevant information about a dataset. The [datasets.load_dataset_builder()](/docs/datasets/master/en/package_reference/loading_methods#datasets.load_dataset_builder) method allows you to inspect the attributes of a dataset without downloading it.
...
&amp;lcub;'text': Value(dtype='string', id=None), 'label': ClassLabel(num_classes=2, names=['neg', 'pos'], names_file=None, id=None)}
>>> print(dataset_builder.info.splits)
&amp;lcub;'train': SplitInfo(name='train', num_bytes=33432835, num_examples=25000, dataset_name='imdb'), 'test': SplitInfo(name='test', num_bytes=32650697, num_examples=25000, dataset_name='imdb'), 'unsupervised': SplitInfo(name='unsupervised', num_bytes=67106814, num_examples=50000, dataset_name='imdb')}
```

<Tip>

Take a look at [datasets.DatasetInfo](/docs/datasets/master/en/package_reference/main_classes#datasets.DatasetInfo) for a full list of attributes you can use with `dataset_builder`.

</Tip>

Once you are happy with the dataset you want, load it in a single line with [datasets.load_dataset()](/docs/datasets/master/en/package_reference/loading_methods#datasets.load_dataset):
...
>>> print(dataset)
&amp;lcub;'train': Dataset(schema: &amp;lcub;'sentence': 'string', 'label': 'int64', 'idx': 'int32'}, num_rows: 67349),
    'validation': Dataset(schema: &amp;lcub;'sentence': 'string', 'label': 'int64', 'idx': 'int32'}, num_rows: 872),
    'test': Dataset(schema: &amp;lcub;'sentence': 'string', 'label': 'int64', 'idx': 'int32'}, num_rows: 1821)
}
```

<h2 id="select-a-split">Select a split</h2>
...
>>> print(datasets)
&amp;lcub;train: Dataset(&amp;lcub;
    features: ['idx', 'label', 'sentence1', 'sentence2'],
    num_rows: 3668
})
validation: Dataset(&amp;lcub;
    features: ['idx', 'label', 'sentence1', 'sentence2'],
    num_rows: 408
})
test: Dataset(&amp;lcub;
...
You can list the split names for a dataset, or a specific configuration, with the [datasets.get_dataset_split_names()](/docs/datasets/master/en/package_reference/loading_methods#datasets.get_dataset_split_names) method:
