...
🤗 Datasets supports access to cloud storage providers through a S3 filesystem implementation: [datasets.filesystems.S3FileSystem](/docs/datasets/master/en/package_reference/main_classes#datasets.filesystems.S3FileSystem). You can save and load datasets from your Amazon S3 bucket in a Pythonic way. Take a look at the following table for other supported cloud storage providers:
