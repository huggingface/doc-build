...
<span class="hljs-addition">+         accelerator.backward(loss)</span>
          ...
          lr_scheduler.step()
          optimizer.zero_grad()
          progress_bar.update(1)`}}),tt=new fr({props:{$$slots:{default:[Er]},$$scope:{ctx:Bt}}}),Nt=new k({props:{code:`from accelerate import Accelerator
from transformers import AdamW, AutoModelForSequenceClassification, get_scheduler
...
<span class="hljs-keyword">for</span> epoch <span class="hljs-keyword">in</span> <span class="hljs-built_in">range</span>(num_epochs):
    <span class="hljs-keyword">for</span> batch <span class="hljs-keyword">in</span> train_dl:
        ...
        lr_scheduler.step()
        optimizer.zero_grad()
        progress_bar.update(<span class="hljs-number">1</span>)`}}),Mt=new k({props:{code:"accelerate config",highlighted:"accelerate config"}}),Wt=new k({props:{code:"accelerate launch train.py",highlighted:'accelerate <span class="hljs-built_in">launch</span> train.py'}}),Lt=new k({props:{code:`from accelerate import notebook_launcher
